...
void event_handler_button(struct _lv_obj_t * obj, lv_event_t event) {
  if(event == LV_EVENT_PRESSED) {
    uint8_t led_wardrope = (obj == wardrope_button ? 0 : 6);
    ...
    uint8_t led_haal = (obj == haal_button ? 24 : 30);
    ...
    set_sideled_color(led_wardrope,led_kitchen, color);
    set_sideled_state(led_wardrope,led_kitchen, state);
  }
}
